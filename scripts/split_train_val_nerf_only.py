...
if __name__ == '__main__':
    ...
    with open(transforms_json) as transforms_file:
        ...
        create_dir(images_train_dir)
        create_dir(images_test_dir)
        ...
        for frame in transforms_frames_train:
            new_path = os.path.join(images_train_dir, os.path.basename(frame["file_path"]))
            shutil.copy(os.path.join(args.scenedir, frame["file_path"]), new_path)
            frame["file_path"] = os.path.relpath(new_path, start=args.scenedir)
        #     os.link(file_path, new_path)
        #     f["file_path"] = new_path
        
        for frame in transforms_frames_val:
            new_path = os.path.join(images_test_dir, os.path.basename(frame["file_path"]))
            shutil.copy(os.path.join(args.scenedir, frame["file_path"]), new_path)
            frame["file_path"] = os.path.relpath(new_path, start=args.scenedir)
