...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="example: python ./scripts/show_llff.py ~/Development/llff\ debug/nuscenes-1/  --visualize --poses_bounds poses_bounds_train.npy")
    ...
    parser.add_argument('--poses_bounds', type=str, default="poses_bounds.npy", help="name of poses_bounds.npy")
    ...
    print(f'[INFO] process {args.path}')
    ...
    # load data
    images = [f[len(args.path):] for f in sorted(glob.glob(os.path.join(args.path, args.images, "*"))) if f.lower().endswith('png') or f.lower().endswith('jpg') or f.lower().endswith('jpeg')]
    
    poses_bounds = np.load(os.path.join(args.path, args.poses_bounds))
